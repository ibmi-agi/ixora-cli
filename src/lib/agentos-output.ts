...
export function outputList(
  ...
  if (format === "json") {
    ...
    // Contract: bare `--json` / `-o json` / piped output emits the rows
    // exactly as passed (the full API payload for most commands) inside
    // the {data, meta} envelope. Filtering fields is opt-in via
    // `--json <fields>`, which emits a flat array. Do not project rows
    // here — scripts rely on the full payload.
