...
> **For exact flags, run `ixora evals <verb> --help`.** This reference covers workflows and eval-type semantics — it does not transcribe every flag.

`ixora evals` creates and inspects eval runs against agents or teams. Verbs: `list`, `get`, `run`, `delete`. Each command targets the resolved system (`--url <url>` bypasses resolution; `--system <id>` picks one).

There is no `update` verb yet — the SDK and server both support renaming an eval run, but the CLI does not surface it.

## evals list

```bash
ixora evals list                            # paginated, newest first
ixora evals list --agent-id ibmi-agent      # filter by component
...
For `accuracy` evals the per-iteration scores live under `eval_data.results[].score` and the judge's rationale under `.reason`. For `reliability` evals look at `eval_data.passed_tool_calls` / `failed_tool_calls`.

## evals run — create + execute synchronously

The eval blocks until the agent/team finishes and (for accuracy / agent_as_judge) until the judge model returns a score. **LLM-judged evals can take 30–120s.**

Always required: `--eval-type`, `--input`, and exactly one of `--agent-id` / `--team-id`.

Type-conditional required flags (CLI rejects mismatches before the server is called):
...
ixora evals run --agent-id ibmi-agent --eval-type accuracy \
  --input "what is 2 plus 2" --expected-output "4"

# Agent-as-judge: another LLM scores the response against your criteria
ixora evals run --agent-id ibmi-agent --eval-type agent_as_judge \
...
ixora evals run --agent-id ibmi-agent --eval-type reliability \
...
ixora evals run --agent-id ibmi-agent --eval-type performance \
  --input "what's the current QSYS library version?" \
  --warmup-runs 1

# Team eval
ixora evals run --team-id my-team --eval-type accuracy \
  --input "..." --expected-output "..."

# Project a couple fields for scripting
...
ixora evals delete --ids $(ixora evals list --json id | jq -r '.[].id' | paste -sd,)
```

## Gotchas

- **`evals run` blocks for the full duration.** LLM-judge runs can take 30–120s; performance/reliability are usually faster but still synchronous. No streaming form exists.
